  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    try {
      ...
      await queryRunner.query(`
        ...
        CREATE INDEX "idx_transactions_user_status_timestamp" ON "transactions" ("user_id", "status", "timestamp");
        CREATE INDEX "idx_transactions_investment_status" ON "transactions" ("investment_id", "status");
        CREATE INDEX "idx_transactions_invoice_status" ON "transactions" ("invoice_id", "status");
      `);
      ...
      logger.info("Successfully executed up migration for InitialSchema1731513600000");
    } catch (error) {
      logger.error("Failed to execute InitialSchema migration up", {
        error: error instanceof Error ? error.message : String(error),
      });
      if (error instanceof AppError) {
        throw error;
      }
      throw new AppError(500, "Initial schema migration failed", "INITIAL_SCHEMA_MIGRATION_FAILED", error);
    }
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    try {
      logger.info("Executing down migration for InitialSchema1731513600000");
      ...
      logger.info("Successfully executed down migration for InitialSchema1731513600000");
    } catch (error) {
      logger.error("Failed to execute InitialSchema migration down", {
        error: error instanceof Error ? error.message : String(error),
      });
      if (error instanceof AppError) {
        throw error;
      }
      throw new AppError(500, "Initial schema migration revert failed", "INITIAL_SCHEMA_REVERT_FAILED", error);
    }
  }
}
